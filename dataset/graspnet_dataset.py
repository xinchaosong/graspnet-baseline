...
class GraspNetDataset(Dataset):
    def __init__(self, root, valid_obj_idxs, grasp_labels, camera='kinect', split='train', num_points=20000,
        ...
        if split == 'train':
            self.sceneIds = list(range(152))
        elif split == 'val':
            self.sceneIds = list(range(152, 171))
        elif split == 'test':
            self.sceneIds = list(range(171, 190))
...
def load_grasp_labels(root):
    ...
    for i, obj_name in enumerate(tqdm(obj_names, desc='Loading grasping labels...')):
        ...
        tolerance = np.load(os.path.join('/media/DATACENTER2/HiLo_other_approaches/GraspNet_1B', 'tolerance',
                                         '{}_tolerance.npy'.format(str(i).zfill(3))))
...
if __name__ == "__main__":
    root = '/media/DATACENTER2/HiLo_other_approaches/GraspNet_1B'
    valid_obj_idxs, grasp_labels = load_grasp_labels(root)
    train_dataset = GraspNetDataset(root, valid_obj_idxs, grasp_labels, split='train', remove_outlier=True,
                                    remove_invisible=True, num_points=20000)
    print(len(train_dataset))
    train_dataset.get_data_label(0)
